...
class Subtitler:
  ...
  def __call__(self, audio):
    ...
    def force_align(SPEECH_FILE, transcript, start_index, start_time):
      ...
      def backtrack(trellis, emission, tokens, blank_id=0):
        ...
        for t in range(t_start, 0, -1):
          ...
          prob = emission[t - 1,
                          tokens[j -
                                 1] if changed > stayed else 0].exp().item()
      ...
      def merge_repeats(path):
        ...
        while i1 < len(path):
          while i2 < len(
              path) and path[i1].token_index == path[i2].token_index:
            i2 += 1
        ...
        while i1 < len(segments):
          if i2 >= len(segments) or segments[i2].label == separator:
            if i1 != i2:
              ...
              score = sum(seg.score * seg.length
                          for seg in segs) / sum(seg.length for seg in segs)
              words.append(
    ...
    for i in range(len(transcription["segments"])):
      inputted_transcript.append(
        st.text_input("Fix transcript",
                      value="".join(transcription["segments"][i]["text"]),
                      key=i))
    ...
    if st.button("Create subtitles"):
      ...
      for i, segment in enumerate(segments):
        text = segment["text"]
        audioSegment = AudioSegment.from_wav(audio)[segment["start"] *
                                                    1000:segment["end"] * 1000]
        audioSegment.export(
          str(i) + '.wav',
